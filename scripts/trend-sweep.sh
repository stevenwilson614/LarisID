...
function fetchJson(path) {
  ...
  return JSON.parse(execSync(cmd, { encoding: 'utf8', maxBuffer: 50*1024*1024 }));
}
...
function ddTrendSelfWeeklyRateFromDeltas(dbRows, corrDeltas) {
  ...
  return rates.length ? rates.sort((a,b)=>a-b)[Math.floor(rates.length/2)] : 0;
}
...
function computeTrend(hist, listing) {
  ...
  if (!skipSyntheticFill) ddTrendFillZeroDeltas(dbRows, corrDeltas, listing, { peerWeeklyRate: impliedRate, selfWeeklyRate });
  let weeks = ddTrendBuildWeeklyRows(dbRows, corrDeltas);
  ...
  return { weeks, corrDeltas, rawSoldFlat, meaningfulRev, atBucket, dbRows, skipSyntheticFill };
}

(async () => {
  const items = fetchJson('listings?select=item_id,shop_id,total_sold,reviews,category,price&scraped_at=lt.2026-06-01&total_sold=gt.0&order=scraped_at.desc&limit=300');
  ...
  const issues = { zeroWeek: [], negWeek: [], perfectlyFlat: [] };
  for (const listing of candidates) {
    const hist = fetchJson(`listings?select=scraped_at,total_sold,price,reviews,category,est_sold,sold_tier&item_id=eq.${listing.item_id}&shop_id=eq.${listing.shop_id}&order=scraped_at.asc&limit=80`);
    ...
    if (units.some(u => u === 0)) issues.zeroWeek.push({ k, units, soldSeries: r.dbRows.map(x=>x.total_sold) });
    if (units.some((u,i) => i>0 && u < units[i-1])) issues.negWeek.push({ k, units });
    if (isPerfectlyFlat(r.weeks)) issues.perfectlyFlat.push({
      k, units, total_sold: listing.total_sold,
      rawSoldFlat: r.rawSoldFlat, meaningfulRev: r.meaningfulRev, atBucket: r.atBucket,
      skipSyntheticFill: r.skipSyntheticFill, deltas: r.corrDeltas,
      soldSeries: r.dbRows.map(x=>({d:x.scraped_at.slice(0,10),s:x.total_sold,r:x.reviews}))
    });
  }
  console.log(JSON.stringify({ checked: candidates.length, counts: {
    zeroWeek: issues.zeroWeek.length,
    negWeek: issues.negWeek.length,
    perfectlyFlat: issues.perfectlyFlat.length,
  }, zeroWeek: issues.zeroWeek.slice(0,3), perfectlyFlat: issues.perfectlyFlat }, null, 2));
})();
NODE
