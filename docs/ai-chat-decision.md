...
> - **Vendor:** Laris AI calls **DeepSeek** (`deepseek-v4-pro`) via `claude-proxy`. The function name is historical.
> - **The recommendation was reversed.** LarisID is now chat-first: `js/gpt-app.js` is the live
>   bundle, Laris AI is a full multi-turn thread, and the AI is a tool-using agent that reads
