...
**Laris AI replies:** [ask-laris.md](./ask-laris.md) — lookup / weekly / filter / refer / promo / judgment. Do not send every typed prompt through visible thinking.
...
**Login → Beranda (`#home-finder`) → Cari Produk / Laris AI → Deep Dive → end-of-dive alert card → Favorit Aku pulse / scrape digest / Langkah minggu ini / Komunitas strip**
...
| WA name capture | `#name-capture` | Required after WhatsApp signup/login if there is no real name. Not skippable. Not onboarding (city/category stay in-page). Also shown on session restore until a name is saved. |
...
| Download-gift bell | `#notif-bell`, `user_notices.kind=export_grant` | people with a 1000-row `user_export_grants` bank; click → Cari Produk. Bell lists the last 20 `user_notices` (unread highlighted). Opening marks `read_at`; it does not hide older rows. |
...
| Edukasi | `#btn-edukasi`, `#view-edukasi`, `#edu-interest-capture` | Logged-in interest yes (name required; missing email or WhatsApp). Popup finishes on the current page; Nanti skips that browser. Not onboarding. Hidden for mentors and people already in a cohort. Platform admins see the page as a new user (dry-run submit); visitor + interest counts live on Admin → Edukasi (`education_admin_overview`, `edu_page_view`). Steven chat chip “Bingung cara pakainya” replies with `#sfb-edu-link` → this tab to daftar bimbingan. |
