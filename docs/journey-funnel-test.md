...
9b. Komunitas: `#btn-community` is in the main sidebar (not under Tentang). Diskusi is the default tab. Signed-in Beranda may show `#home-komunitas` (Pertanyaan minggu ini + belum terjawab) and must stay hidden when empty — not a popup. Deep Dive Aksi Cepat and Cari Produk `#dir-ask-sellers` open Diskusi with a keyword-only title (no harga/omset). `?komunitas=<uuid>` deep-links the thread after login.
...
13. Laris AI (logged in): `Crocs` → listing rows + short Indonesian overview, no “Proses berpikir”. `Botol Minyak` → overview of the oil-bottle pasar + listing rows; chips only markets that contain both tokens (`botol minyak dispenser kaca`, `botol plastik kemasan minyak`); **not** sabun cuci botol bayi / sterilizer / kecap; default chip Semua; no empty table; no Trending Sekarang in that bubble. Typed and chip “Apa yang terlaris minggu ini?” → reviewing steps then ~10 rows. Follow-up “ada yang dari Bandung?” → subset of those rows. “untuk affiliate…” → Sinyal Promosi (perkiraan) + listing rows + user-rate calculator; footnotes say no komisi / XTRA / jumlah afiliator. “affiliate tiktok” / Kalodata → short Kalodata handoff, no Shopee rows. Judgment question still shows thinking. “Lanjutkan jawaban” continues, does not filter. Profit chip still opens the calculator. Deep Dive fee card says “biaya platform (bukan komisi afiliasi)”.
...
15. Cari Produk search (bar Enter, not only a suggestion click):
    - `sendok bayi silikon`: page 1 is feeding-spoon / MPASI rows whose titles contain `sendok`; lead says judulnya cocok and names `peralatan mpasi set`; Urutkan shows Paling sesuai; no stroller, popok, spatula, or `kursi camping`. Switching Urutkan to Omset reorders the same pool (strollers do not reappear).
    - `rayban` / `ray-ban`: titles mentioning Ray-Ban + kacamata/sunglasses first, then other kacamata from planner type markets (not kaca film / riben); lead is merek dulu, lalu produk mirip in a kacamata pasar; no resmi/original claim. Same string in Laris AI uses the Semua chip.
