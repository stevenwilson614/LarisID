...
8b. Favorit Aku page: list shows photo / harga / omset / one % / sparkline; counter `n / 30`; Tambah typeahead saves on click; notify strip is Email / WhatsApp + cadence (Setiap ada perubahan / Sekali seminggu) with an honest WA status line; per-row Pantau toko switch; no 4-step wizard. Empty state is one line + Tambah + link to Cari Produk. New favorite without history says “Data harian mulai besok”.
...
14. Cari Produk Trending Sekarang (`#dir-trending-now` and chat `.trend-host`): rank-1 row is ~50% larger; mascots are trophy / silver / bronze (not the search card); titles one line with ellipsis; harga is larger than omset; weekly % is followed by a tilted lightning bolt. Row click still opens Deep Dive.
