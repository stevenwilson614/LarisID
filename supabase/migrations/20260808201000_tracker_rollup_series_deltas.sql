-- Widen get_tracker_rollup series with avg_price / n_listings / n_sellers so
-- the Pantauan row-detail "Riwayat Perubahan" can show day-over-day moves.
-- Also raise the p_days cap 30 → 90 to match the UI's 7/30/60/90 selector,
-- and include all current-window scrape days (not only sold_delta > 0).
    ...
    from (
      select
        ...
        coalesce((
          ...
          from win s where s.keyword = t.keyword and s.is_cur
        ), '[]'::json)                     as series
      from tracked t
    ...
    from (
      select
        ...
        coalesce((
          ...
          from win s where s.shop_id = t.shop_id and s.is_cur
        ), '[]'::json)                     as series
      from tracked t
