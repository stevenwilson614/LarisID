-- Drop offtopic members from product-type cards (Rule A already stamped
-- is_offtopic on listings_deduped) and demote kw_hits = 0 rows out of the
-- cover slot when the token test is reliable for that keyword (rel_share >= 0.15).
--
-- Body is the live mv_product_types definition (structurally the 20260723
-- file; Postgres expanded names/casts) with those two edits.
...
ranked as (
  select r.*,
         ...
         row_number() over (
           ...
           order by (case when rel.rel_share >= 0.15 and r.kw_hits = 0 then 1 else 0 end),
...
imgs as (
  select keyword, city,
         (array_agg(image_url
            order by (case when rel_share >= 0.15 and kw_hits = 0 then 1 else 0 end),
...
-- listings_deduped is now ~860k rows and evaluates _lid_kw_hits on refresh;
-- 900s was the previous cap and is tight for a full breakout rebuild.
alter function public.refresh_breakout_matviews() set statement_timeout to '3600s';
