-- Expose WhatsApp number on admin_user_directory for the Admin Users table.
-- Prefer profile wa_number, then public_whatsapp, then tracker notify number.
...
as $$
  select
    ...
    coalesce(
      ...
      nullif(btrim(st.notify_wa_number), '')
    )::text
  from auth.users u
