-- Admin Users: don't show a WA phone as "Nama", and fill WA from the signup
-- identity (auth.users.phone / @wa.larisid.com email) when the profile column
-- was never written. Also backfill user_profiles.wa_number for those accounts.
...
as $$
  select coalesce(
    length(regexp_replace(coalesce(p, ''), '[^0-9]', '', 'g')) between 9 and 15
    and regexp_replace(coalesce(p, ''), '[^0-9]', '', 'g') ~ '^(62|0?8)[0-9]+$'
    and regexp_replace(coalesce(p, ''), '[\s+()-]', '', 'g') ~ '^[0-9]+$',
    false
  );
$$;

revoke all on function public.is_phone_like_name(text) from public;

insert into public.user_profiles (user_id, wa_number, updated_at)
select
  u.id,
  coalesce(
    public.normalise_wa_phone(u.phone),
    public.normalise_wa_phone(u.raw_user_meta_data ->> 'phone_number'),
    public.normalise_wa_phone(split_part(u.email, '@', 1))
  ),
  now()
from auth.users u
where u.email ilike '%@wa.larisid.com'
  and coalesce(
    public.normalise_wa_phone(u.phone),
    public.normalise_wa_phone(u.raw_user_meta_data ->> 'phone_number'),
    public.normalise_wa_phone(split_part(u.email, '@', 1))
...
set wa_number = coalesce(nullif(btrim(user_profiles.wa_number), ''), excluded.wa_number),
    updated_at = now()
where user_profiles.wa_number is null or btrim(user_profiles.wa_number) = '';
