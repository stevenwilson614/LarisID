-- ============================================================================
-- Split export budgets:
--   snapshot (Cari Produk)  -> export_rows_used  / 90 product rows / day
--   history  (Deep Dive)    -> export_weeks_used / 12 weeks / day
--
-- Snapshot still costs 1 per product (monthly omset). History costs the number
-- of weeks requested (not measured-only) so the UI "12/12 minggu tersisa" is
-- honest and predictable.
--
-- Apply: bash scripts/apply-selfhost.sh supabase/migrations/20260909140000_export_weeks_budget.sql
-- ============================================================================

begin;

alter table public.daily_usage
  add column if not exists export_weeks_used integer not null default 0;

comment on column public.daily_usage.export_weeks_used is
  'Weeks of omset history written into Deep Dive exports today (WIB). '
  'Independent of export_rows_used (Cari Produk snapshot rows).';

create or replace function public._export_week_limit()
returns integer language sql immutable as $$ select 12 $$;

comment on function public._export_week_limit() is
  'Daily weeks budget for Deep Dive history exports. EXPORT_WEEK_LIMIT in '
  'js/gpt-app.js is display-only; the server decides.';

revoke all on function public._export_week_limit() from public, anon, authenticated;
...
revoke all on function public.get_my_export_quota() from public, anon;
grant execute on function public.get_my_export_quota() to authenticated;

-- ---------------------------------------------------------------------------
-- Metered export (dual budget)
-- ---------------------------------------------------------------------------
...
begin
  ...
  -- History is product-capped (payload/query bound) and week-budgeted separately.
  ...
  if v_prior.request_id is not null then
    ...
    if v_hist then
      ...
      -- Cap the requested window to what the user still has today.
  ...
  -- Resolve snapshot rows once, in the client's order.
  ...
  if v_hist then
    -- History: keep all resolved products; charge weeks * products against the
    -- weeks budget. Deep Dive is always 1 product, so cost = v_weeks.
    ...
    -- Snapshot: 1 per product against the row budget.
    with snap as (
...
comment on function public.export_rows(uuid, bigint[], bigint[], text[], boolean, int, text) is
  'Meters and returns XLSX/CSV export rows. Snapshot charges product rows '
  '(90/day); history charges weeks (12/day). Idempotent on (auth.uid(), '
  'p_request_id). Does NOT consult _beta_unlimited().';

-- create or replace preserves ACL; keep authenticated-only.
