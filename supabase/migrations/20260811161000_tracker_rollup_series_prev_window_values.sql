-- Fixes a bug in 20260811160000: when `series` was widened to include the
-- previous window's points (is_cur = false), it kept reading `s.omset_delta`
-- / `s.sold_delta` for every point — but those columns are computed with the
-- CURRENT window's overlap fraction (_win_overlap(d, span, v_cur0, now)).
-- For an older date whose bucket doesn't reach into the current window, that
-- overlap is 0, so every previous-window point in `series` silently read as
-- 0 units/0 omset regardless of what actually happened that day. The chart
-- then drew a flat near-zero line for the whole "history" half and a sudden
-- jump at the current-window boundary — an artifact of reading the wrong
-- column, not a real trend. Confirmed 2026-08-11 testing the widened series
-- against real data for a tracked keyword.
--
-- Fix: pick the delta column that matches which window each point actually
-- belongs to — s.omset_delta/s.sold_delta for is_cur points (unchanged),
-- s.omset_delta_prev/s.sold_delta_prev for the rest.
