...
-- - stevenfwilson@gmail.com = cohort leader (Ocean Blue)
...
-- 2) stevenfwilson@gmail.com must exist under Authentication → Users (sign up once in the app).
...
values
  ...
  ('stevenfwilson@gmail.com', 'leader', 'Cohort leader'),
...
-- Set mentor (cohort leader) to stevenfwilson@gmail.com for Ocean Blue.
update public.cohorts c
set mentor_user_id = u.id
from auth.users u
where lower(u.email) = 'stevenfwilson@gmail.com'
  and c.slug = 'ocean-blue';

insert into public.cohort_members (cohort_id, user_id, role, status)
select c.id, u.id, 'mentor', 'active'
from public.cohorts c
join auth.users u on lower(u.email) = 'stevenfwilson@gmail.com'
...
-- update public.cohorts c
-- set mentor_user_id = (select id from auth.users where email = 'stevenfwilson@gmail.com' limit 1)
