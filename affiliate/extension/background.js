...
function bestFrame() {
  ...
  list.sort(function (a, b) {
    var as = (a.affiliate ? 4 : 0) + (a.recon && (a.recon.collab || a.recon.im) ? 2 : 0) + (isAffiliateUrl(a.href) ? 1 : 0);
    var bs = (b.affiliate ? 4 : 0) + (b.recon && (b.recon.collab || b.recon.im) ? 2 : 0) + (isAffiliateUrl(b.href) ? 1 : 0);
    return bs - as;
  });
  return list[0] || null;
}
...
function pingPayload() {
  var f = bestFrame();
  if (!f || !f.affiliate) {
    return { ok: false, code: 'no_tab', reason: 'Buka Affiliate Center di tab Chrome ini.', recon: (f && f.recon) || {} };
  }
  return {
    ok: true,
    connected: true,
...
function sendToBestFrame(message) {
  return new Promise(function (resolve) {
    var f = bestFrame();
    if (!f || !f.affiliate) {
      resolve({ ok: false, code: 'no_tab', reason: 'Buka Affiliate Center di tab Chrome ini.' });
      return;
    }
